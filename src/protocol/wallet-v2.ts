...
export class WOTSWallet implements WOTSWalletJSON {
    ...
    public v3address: WotsAddress | null;
    ...
    private constructor({
        ...
        this.v3address = this.address?  WotsAddress.wotsAddressFromBytes(this.address) : null;
    }
    ...
    getAddr(): ByteArray | null {
        return this.address ? new Uint8Array(this.address.slice(WOTS.WOTSSIGBYTES, WOTS.WOTSSIGBYTES + 32)) : null;
    }
    ...
    static createV3(name: string, secret: ByteArray, v3tag: ByteArray) {
        const { private_seed } = this.componentsGenerator(secret);
        const sourcePK = WOTS.generateAddress(v3tag, secret, this.componentsGenerator);
        const ww = new WOTSWallet({ name, address: sourcePK, tag: v3tag, secret: private_seed });
        return ww;
    }
